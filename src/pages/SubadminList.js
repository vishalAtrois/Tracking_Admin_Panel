 ...
 const [sidebarOpen, setSidebarOpen] = useState(false);
  ...
  function fetchUsers() {
    ...
    const url = searchQuery
      ? `https://tracking-backend-admin.vercel.app/v1/admin/searchSubAdmin?query=${searchQuery}&page=${currentpage}&limit=${limit}`
      : `https://tracking-backend-admin.vercel.app/v1/admin/fetchSubAdminList?page=${currentpage}&limit=${limit}&sortBy=created%3Adesc`;
      ...
      {loading ? (
        ...
        <table className="min-w-full table-auto bg-white text-black text-sm">
          <thead className="bg-white">
            <tr>
              {['Sr.no', 'Name', 'Email', 'Mobile Number', 'Company Name'].map((heading) => (
